...
function SetClothing(drawables, props, drawTextures, propTextures, keepHair)
    for i = 1, #drawable_names do
		if i == 3 and keepHair == true then
			-- do nothing
		else
...
AddEventHandler("setClothes", function(data)
    player = PlayerPedId()
	if GetEntityModel(player) == `a_c_humpback` then
    ...
    SetClothing(data.drawables, data.props, data.drawtextures, data.proptextures, true)
end)

AddEventHandler('loadClothing', function(data, default)
	data.model = tonumber(data.model)
	SetSkin(data.model, default)
...
RegisterNUICallback('customskin', function(data, cb)
    if canUseCustomSkins() then
        ...
        if valid_model or (IsModelValid(hash) and IsModelAPed(hash)) then
            SetSkin(hash, true)
        end
    end
end)
...
AddEventHandler("betrayed_skins:setclothes", function(data,alreadyExist)
	...
	if (not data or not data.model) and alreadyExist == nil then setDefault() return end
...
AddEventHandler("betrayed_skins:defaultReset", function()
	...
	while not gender do Citizen.Wait(10) end
    Citizen.Wait(500)
    if gender ~= "m" then
        SetSkin(`mp_f_freemode_01`, true)
    else
...
RegisterNetEvent("betrayed_skins:characterCreation")
AddEventHandler("betrayed_skins:characterCreation", function(isMale)
	enabled = true
    Citizen.Wait(500)
    if isMale then SetSkin(`mp_m_freemode_01`, true)
	else
        SetSkin(`mp_f_freemode_01`, true)  
    end
    startingMenu = true
    OpenMenu("clothesmenu")
	while enabled do Citizen.Wait(0) end
	enabled = true
	Citizen.Wait(500)
	local model = GetEntityModel(PlayerPedId())
	if model == `mp_f_freemode_01` or model == `mp_m_freemode_01` then
...
function MenuOpen() return enabled end
exports('MenuOpen', MenuOpen)
exports('CloseMenu', EnableGUI)
...
AddEventHandler('betrayed_skins:listOutfits', function(skincheck)
    exports['mythic_notify']:DoLongHudText('inform', 'Here you can use /outfitadd, /outfituse or /removeoutfit')
