...
function ManageFuelUsage(vehicle)
	...
	local fuelLevel = GetVehicleFuelLevel(vehicle) or 100.0

	if IsVehicleEngineOn(vehicle) then
		SetFuel(vehicle, fuelLevel - Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle))] * (Config.Classes[GetVehicleClass(vehicle)] or 1.0) / 10)
	end
end
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		local fuelToAdd = math.random(1, 2)
