...
AddEventHandler("Pass-Alarm:Return:NewPanic", function(source, Officer)
	...
	if xPlayer.job.name == "police" or xPlayer.job.name == "ambulance" then
		...
		TriggerServerEvent("tp:addChatPolice", "[10-13] ".. Officer.Name .." Is down on  " .. Officer.Location, 45000)
...
RegisterNetEvent("Pass-Alarm:Return:MedicNewPanic")
AddEventHandler("Pass-Alarm:Return:MedicNewPanic", function(source, Officer)
	local _source = source
	local xPlayer =ESX.GetPlayerData()
	local playername = xPlayer.name
	...
	if xPlayer.job.name == "police" or xPlayer.job.name == "ambulance" then
		...
		-- Only people tuned to the panic channel can see the message
		TriggerServerEvent("tp:addChatPolice", "[10-14] ".. Officer.Name .." Is down on  " .. Officer.Location, 45000)
...
RegisterNetEvent("tp:panicTrigger")
AddEventHandler("tp:panicTrigger", function()
    local _source = source
	local xPlayer =ESX.GetPlayerData()
	local playername = xPlayer.name
	...
	if xPlayer.job.name == "police" then
        if Panic.Cooling == 0 then
			...
			TriggerEvent('wl-alerts:civilian:notify', 'officerdistress')
			TriggerEvent('tfbroke:civilian:notify', 'pdisdeadboy')
...
RegisterNetEvent("tp:panicTriggerMedic")
AddEventHandler("tp:panicTriggerMedic", function()
    local _source = source
	local xPlayer =ESX.GetPlayerData()
	local playername = xPlayer.name
	...
	if xPlayer.job.name == "ambulance" then
        if Panic.Cooling == 0 then
			...
			TriggerEvent('wl-alerts:civilian:notify', 'emsdistress')
            TriggerServerEvent("Medic-Panic:NewPanic", Officer)
