...
local TicketMachines = {`prop_train_ticket_02`, `prop_train_ticket_02_tu`, `v_serv_tu_statio3_`, `dt1_21_stat`, `dt1_21_stat+hi`, `dt1_21_station+hidr`, `metro_stationhut002seoul`, `v_serv_metro_paybooth`}
...
Citizen.CreateThread(function()
	...
	function StartTrain()
		...
		Wait(1500) -- Added a small 'waiting' while the train is loaded (to prevent the)
		...
		SetTrainCruiseSpeed(MetroTrain, 25.0)
		...
		if UseTwoMetros == 1 then
			...
			SetTrainCruiseSpeed(MetroTrain2, 25.0)
		end
		Wait(200) -- Added a small 'waiting' while the train is loaded (to prevent the)
				  -- random unexplained spawning of the freight train on the Metro Rails

		TrainDriverHash = `s_m_m_lsmetro_01`
		...
		if IsControlJustPressed(0, 51) then
			...
			if not IsPlayerNearMetro and not IsPlayerInMetro and not SkipReEnterCheck then
				if not IsPlayerInVehicle then
					...
					local coordB = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 3.0, 0.0)
					local Metro = getVehicleInDirection(coordA, coordB)
					if DoesEntityExist(Metro) then
						if GetEntityModel(Metro) == `metrotrain` then
							if not PlayerHasMetroTicket	then
								...
								if IsPlayerWantedLevelGreater(PlayerId(), 0) and AllowEnterTrainWanted == 0 then
									...
									if FreeWalk then
										...
										SetPedIntoVehicle(PlayerPedId(), CurrentMetro, 1)
									end
			...
			if not IsPlayerInVehicle then
				...
				if not IsPlayerNearTicketMachine then
					for k,v in pairs(TicketMachines) do
						TicketMachine = GetClosestObjectOfType(x, y, z, 0.75, v, false)
...
function StopTrain(train)
	if (NetworkHasControlOfEntity(train)) then
		...
		Citizen.Wait(100)
		SetTrainCruiseSpeed(train, 25.0) -- Bye bye train !
