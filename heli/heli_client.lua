...
local toggle_helicam = 74 -- control id of the button by which to toggle the helicam mode. Default: INPUT_CONTEXT (E)
...
local radiusup_key = 61 -- control id to increase manual spotlight radius. Default: INPUT_VEH_PUSHBIKE_SPRINT (CAPSLOCK)
local radiusdown_key = 62 -- control id to decrease spotlight radius. Default: INPUT_SPRINT (LEFT-SHIFT)
...
local helicam = false
local polmav_hash = {[1] = `polmav`, [2] = `frogger2`, [3] = `avenger`}
	...
	while true do
		...
		if IsPlayerInPolmav() then
			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) and DoesVehicleAllowRappel(heli) then -- Initiate rappel
		...
		if helicam then
			...
			if IsVehicleModel(heli, `avenger`) then AttachCamToEntity(cam, heli, 0.0,0.0,-2.2, true)
			else AttachCamToEntity(cam, heli, 0.0,0.0,-1.5, true) end
...
function IsPlayerInPolmav()
	...
	for k,v in pairs(polmav_hash) do
		if IsVehicleModel(vehicle, v) then return true end
	end
	return false
end
