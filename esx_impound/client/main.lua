...
AddEventHandler('esx:setJob', function(job)
	...
	if DoesEntityExist(vehicle) then
	  if hasImpoundAppropriateJob() then
		if IsPedInAnyVehicle(PlayerPedId(), false) then
		  exports['mythic_notify']:SendAlert('error', 'You can not use /impound when in a vehicle.',7000)
		else
		  ...
		  ESX.TriggerServerCallback('esx_impound:impound_vehicle', function()
			exports['mythic_notify']:SendAlert('inform', 'Vehicle has been impounded',7000)
			ESX.Game.DeleteVehicle(vehicle)
		  end, plate)
		end
	  else
		exports['mythic_notify']:SendAlert('error', 'You do not have permission to use this command',7000)
		end
	  end
  ...
  RegisterNetEvent("tp_impound:impounded")
  AddEventHandler("tp_impound:impounded", function(source)
	  ImpoundCurrentVehicle()
  end)
  
  function ImpoundCurrentVehicle()
	  if currentJob == 'police' or currentJob == 'mechanic' then
	  ...
	  exports["t0sic_loadingbar"]:StartDelayedFunction('Impounding vehicle', 10000, function() end)
	  Citizen.Wait(10000)
  ...
  function loadListVehiclePage(elementPages, page)
		  ...
		  if Config.UserMustPayFine then
			ESX.TriggerServerCallback('esx_impound:check_money', function(paidFine)
			  if paidFine then
				...
				exports['mythic_notify']:SendAlert('error', 'You do not have enough money to retrieve your vehicle yet, please try again soon.',7000)
				end
			  end)
			else
			  SpawnVehicle(data.current.value.vehicle)
	...
	function SpawnVehicle(vehicle)
	  ...
	  ESX.TriggerServerCallback('esx_impound:retrieve_vehicle', function()
		exports['mythic_notify']:SendAlert('error', 'Vehicle has been released',7000)
