...
function UnJail()
	...
	TriggerServerEvent('betrayed_skins:get_character_current')
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Cook for inmates"
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("You've already grabbed the food")
							end
...
function PackPackage(packageId)
	...
	LoadModel("prop_food_bs_tray_02")

	local PackageObject = CreateObject(GetHashKey("prop_food_bs_tray_02"), 1716.8, 2576.01, 46.38 - 1.0, true)
	...
	while Packaging do
		...
		if not IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_BBQ") then
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Prepping food... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Put Food out", 0.4)
