...
function OpenMobileMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		elements = {
			...
			{label = _U('jack_repair'),	  value = 'jack_repair'},	
		...
		elseif data.current.value == 'jack_repair' then 											--Jack Stand to F6 menu functions
...
RegisterNetEvent('esx_mechanicjob:onClean')
AddEventHandler('esx_mechanicjob:onClean', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local veh, distance = ESX.Game.GetClosestVehicle(coords)
...
AddEventHandler('esx_mechanicjob:onHijack', function()
	ESX.TriggerServerCallback('esx_mechanicjob:getItemAmount', function(quantity)
		if quantity > 0 then
			local playerPed = PlayerPedId()
			local coords    = GetEntityCoords(playerPed)
			...
			if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
				...
				if DoesEntityExist(vehicle) then
					...
					Citizen.CreateThread(function()
						...
						TriggerServerEvent('esx_mechanicjob:removeItem', 'blowpipe')
					end)
				end
			end
		else
			ESX.ShowNotification("You do not ~r~have enough~s~ Blowtorches")
		end
	end, 'blowpipe')
end)
			...
			if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
				...
				if DoesEntityExist(vehicle) then
					...
					Citizen.CreateThread(function()
						...
						TriggerServerEvent('esx_mechanicjob:removeItem', 'carokit')
					end)
				end
			end
		else
			ESX.ShowNotification("You do not ~r~have enough~s~ Body Kits")
			end
	end, 'carokit')
end)
			...
			if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
				...
				if DoesEntityExist(vehicle) then
					...
					Citizen.CreateThread(function()
						...
						TriggerServerEvent('esx_mechanicjob:removeItem', 'fixkit')
					end)
				end
			end
		else
			ESX.ShowNotification("You do not ~r~have enough~s~ Repair Kits")
		end
	end, 'fixkit')
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 178) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			if NPCOnJob then
...
RegisterNetEvent('nk_repair:JackRepair')
AddEventHandler('nk_repair:JackRepair', function(ped, coords, veh)
	local dict
	...
	exports['progressBars']:startUI(9250, "POSITIONING JACK") -- TRANSLATE THIS, THAT SAY WHEN YOU PUT THE CRIC
	...
	exports['progressBars']:startUI(11000, "REPAIRING VEHICLE") -- TRANSLATE THIS - THAT SAY WHEN YOU REPAIR THE VEHICLE
	...
	SetVehicleEngineOn(vehicle, true, true)
	ClearPedTasksImmediately(playerPed)
	exports['progressBars']:startUI(8250, "REMOVING JACK") -- TLANSTALE THIS - THAT SAY WHEN YOU LEAVE THE CRIC
	TaskPlayAnimAdvanced(ped, dict, 'car_bomb_mechanic', coords, 0.0, 0.0, headin, 1.0, 0.5, 1250, 1, 0.0, 1, 1)
	Citizen.Wait(1250)
	SetEntityCoordsNoOffset(veh, vehpos.x, vehpos.y, vehpos.z + 0.4, true, true, true)
	TaskPlayAnimAdvanced(ped, dict, 'car_bomb_mechanic', coords, 0.0, 0.0, headin, 1.0, 0.5, 1000, 1, 0.25, 1, 1)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(50)
        if v == 'n' and IsPedSittingInAnyVehicle(PlayerPedId()) then
            v = 's'
            checkluzes()
        elseif v == 's' and IsPedOnFoot(PlayerPedId()) then
            v = 'n'
        end
    end
end)
