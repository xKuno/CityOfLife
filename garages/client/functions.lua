...
SpawnLocalVehicle = function(data)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		exports['mythic_notify']:SendAlert('inform', 'Please move the vehicle off the road')
...
SpawnVehicle = function(data,recuperar)
    ...
    if recuperar then
        exports['mythic_notify']:SendAlert('success', 'You paid $2000 to recover your vehicle!')
...
SaveInGarage = function(garage)
    ...
    exports['mythic_notify']:SendAlert('success', 'You saved your vehicle in garage '..garage)
    Citizen.Wait(500)
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return exports['mythic_notify']:SendAlert('inform', 'This model does not exist in the game, send a report!')
    end
